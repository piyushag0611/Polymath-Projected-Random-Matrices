...
Phi = np.array([[2, 1, 1, -1], 
                [1, 2, 1, -1],
                [1, 1, 2, -1], 
                [-1, -1, -1, 2]])

for i in range(n):
    X_goe = gen_CGSE(2, Phi)
...
ax.set_ylabel("d")
ax.set_zlabel("b")
plt.title("CGSE v.s. exponentiated CGSE")
...
plt.ylabel("$\lambda_1$")
plt.title("Eigenvalues Visualization For Exponentiated \n CGSE Matrices With $\Sigma = I$ and $\lambda_1>\lambda_2$")

plt.xlim(0, 7)
# plt.ylim(0,5)
# plt.show()
plt.savefig("CGSE_I_1.png")
...
plt.xlabel("$\lambda_2(<\lambda_1)$")
plt.ylabel("$\lambda_1$")
plt.title("Eigenvalues Visualization For Exponentiated CGSE Matrices \n With $\Sigma = [[1 -1], [-1 2]]$ and $\lambda_1>\lambda_2$")

plt.xlim(0, 10)
plt.ylim(0, 10)
# plt.show()
plt.savefig("CGSE_notI_1.png")
